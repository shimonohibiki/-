...
class ImageDataset(Dataset):
    ...
    def __init__(self, image_dir: str, image_size: int, mean: list, std: list, mode: str) -> None:
        ...
        if self.mode == "Train":
            # Use PyTorch's own data enhancement to enlarge and enhance data 画像の前処理（回転とか）
            self.pre_transform = transforms.Compose([
                transforms.RandomResizedCrop(self.image_size),#ランダムな位置から指定サイズでクロップし、リサイズします。
                TrivialAugmentWide(),
            ...
            self.pre_transform = transforms.Compose([
                ...
                transforms.CenterCrop([self.image_size, self.image_size]),#画像全体を見てる
            ])
        else:
    ...
    # 処理された画像テンソルとそのクラスラベルを辞書形式で返します。
    ...
    # データセット内の総アイテム数（この場合は画像の総数）を返します。
