...
def main():
    ...
    # load_dataset()関数を呼び出して、トレーニングと検証用のデータローダーを取得します。
    train_prefetcher, valid_prefetcher = load_dataset()
    print(f"Load `{config.model_arch_name}` datasets successfully.")

    # build_model()関数で、ResNetモデルとそのEMA（Exponential Moving Average）バージョンを作成します。
...
# Cosine Annealing with Warm Restarts学習率スケジューラを定義します。
...
def train(
...
) -> None: # (アノテーション)これは関数を作る際に、引数や返り値の型を明確に記述するためにある機能です、処理には影響しない
    ...
    while batch_data is not None:
        ...
        # top1, top5 = accuracy(output, target, topk=(1, min(5, config.model_num_classes)))
        top1, _ = accuracy(output, target, topk=(1, min(3, config.model_num_classes)))
...
def validate(
    ...
    acc3 = AverageMeter("Acc@3", ":6.2f", Summary.AVERAGE)
    progress = ProgressMeter(batches, [batch_time, acc1, acc3], prefix=f"{mode}: ")
    ...
    with torch.no_grad():
        while batch_data is not None:
            ...
            top1, _ = accuracy(output, target, topk=(1, min(3, config.model_num_classes)))
            acc1.update(top1[0].item(), batch_size)
            # acc3.update(top3[0].item(), batch_size)
