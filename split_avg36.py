...
def average_6x6_stride6(matrix):
    rows, cols = matrix.shape
    result_rows = (rows + 5) // 6  # 切り上げ除算
    result_cols = (cols + 5) // 6  # 切り上げ除算
    ...
    for i in range(result_rows):
        for j in range(result_cols):
            # 6x6の窓の範囲を定義（ストライド6）
            row_start = i * 6
            row_end = min(row_start + 6, rows)
            col_start = j * 6
            col_end = min(col_start + 6, cols)
...
for k in range(13):
    for j in range(17):
        ...
        # 新しいディレクトリを作成
        output_dir = f"C:\\Users\\hibik\\Desktop\\kenkyu\\dataset\\heatmap_avg36\\S{k+1}\\{j+1}"
        ...
        for i in range(len(lines_sp)):
            ...
            # 近傍16つのマスを平均
            averaged_matrix = average_6x6_stride6(matrix)
